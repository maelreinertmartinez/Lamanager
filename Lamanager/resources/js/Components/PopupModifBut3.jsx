...
function PopupModifBut3({ onClose, promos, selectedYear }) {
    const but3Promo = promos.find(promo => promo.nom === 'BUT 3');
    const [groupesData, setGroupesData] = useState([]);

    useEffect(() => {
        const fetchGroupes = async () => {
            try {
                const response = await axios.get(`/api/groupes/${but3Promo.id}`);
