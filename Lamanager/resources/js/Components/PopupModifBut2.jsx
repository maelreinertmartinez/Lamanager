...
function PopupModifBut2({ onClose, promos }) {
    const but2Promo = promos.find(promo => promo.nom === 'BUT 2');
    const [groupesData, setGroupesData] = useState([]);

    useEffect(() => {
        const fetchGroupes = async () => {
            try {
                const response = await axios.get(`/api/groupes/${but2Promo.id}`);
